<template>
    <li>
        <a :href="'/tasks/' + task.id + '/edit'" class="flex items-center justify-between px-4 py-4 hover:bg-gray-50 sm:px-6">
            <div class="flex items-center truncate space-x-3">
                <div class="w-2.5 h-2.5 flex-shrink-0 rounded-full bg-pink-600"></div>
                <p class="font-medium truncate text-sm leading-6">{{task.name}} <span
                    class="truncate font-normal text-gray-500" v-text="task.arguments" /></p>
            </div>
            <svg
